...
def give_layout_for_outer_tabs(value):
    ...
    if value==2:
        ...
        return html.Div([dcc.Input(id='search', 
                                           value='Search', type='text',
                                           style={'width': '49%','align':'center'}),
                                           tab],style={'align':'center','margin-left': '457px'})
    return None
...
@app.callback(Output('search-results', 'rows'),[Input('search','value')] ) 
def search_db(text):
    res=se.search_index('agent_name',text)
    print(res)
    df=pd.DataFrame(res,index=range(int(len(res)/4)))
    
    tab=dt.DataTable(id='search-result-table',
                                     row_selectable=True,
                                     columns=list(sch_dict.keys()),
                                     selected_row_indices=[])
